...
// Import de imágenes usando @ para evitar errores de ruta
...
const handleFileUpload = (event) => {
  ...
  reader.onload = (e) => {
    const fileMsg = {
      ...
      type: 'file',
      fileName: file.name,
      fileData: e.target.result, // Aquí se guarda el Base64
    ...
    // Respuesta automática ante un archivo
...
// Función para la respuesta del experto corregida
const simulateBotResponse = (responseText) => {
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      allMessages.value[selectedContact.value.name].push(response);
      saveHistory();
      scrollToBottom();
    }, 2000);
  }, 1000);
...
.message-bubble {
  max-width: 65%; padding: 12px 18px; border-radius: 18px; font-size: 14px; position: relative; line-height: 1.5;
}
...
/* ESTILOS DE ARCHIVOS DENTRO DEL CHAT */
...
.send-btn {
  background: #0097B2; color: white; border: none; width: 48px; height: 48px; border-radius: 14px; cursor: pointer;

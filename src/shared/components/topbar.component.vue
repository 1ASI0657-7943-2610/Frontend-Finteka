  ...
  <header class="topbar">
    <div class="search-container">
      ...
      <input
          ...
          @blur="hideDropdown"
          placeholder="Buscar expertos, especialidades..."
          class="search-input"
      />
      ...
      <div v-if="isSearchFocused && searchSuggestions.length > 0" class="search-dropdown">
        ...
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="suggestion-icon">
            <circle cx="11" cy="11" r="8"></circle>
            <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
          </svg>
          {{ item }}
...
const searchSuggestions = computed(() => {
  ...
  return availableSuggestions.filter(item => item.toLowerCase().includes(query));
});
...
.pay-input { width: 100%; box-sizing: border-box; padding: 12px; border: 1px solid #E5E7EB; border-radius: 10px; outline: none; font-family: inherit; font-size: 14px; }
...
.suggestion-icon { width: 16px; height: 16px; color: #9CA3AF; }
